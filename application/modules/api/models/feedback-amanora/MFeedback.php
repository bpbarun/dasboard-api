 ...
 * @author          Barun Pandey
 * @date            9th June, 2020, 11:40:00 PM
 * @version         1.0.0
 */
class mFeedback extends CI_Model {
    ...
    public function getData($id) {
        try{
             if (!empty($id)) {
              $data = $this->db->get_where("amanora_feedback", ['feedback_id' => $id])->row_array();
        } else {
            $this->db->order_by('feedback_id', 'DESC');
            $data = $this->db->get("amanora_feedback")->result();
        }
        if (!empty($data)) {
    ...
    public function insertData($input) {
        try{
            $data = $this->db->insert('amanora_feedback', $input);
        if (!empty($data)) {
            $response['status'] = TRUE;
            $id = $this->db->insert_id();
            $q = $this->db->get_where('amanora_feedback', array('feedback_id' => $id));
            $response['data'] = $q->row();
        } else {
    ...
    public function updateData($id, $input) {
        try{
          $data = $this->db->update('amanora_feedback', $input, array('feedback_id' => $id));
    ...
    public function deleteData($id) {
        try{
            $data = $this->db->delete('amanora_feedback', array('feedback_id' => $id));
