 ...
 * @version         1.0.0
 */
class mAlbumAssets extends CI_Model {
    ...
    public function getData($id) {
        if (!empty($id)) {
            $data = $this->db->get_where("album_asset", ['asset_id' => $id])->row_array();
        } else {
            $this->db->order_by('asset_id', 'ASC');
            $data = $this->db->get("album_asset")->result();
        }
        if (!empty($data)) {
    ...
    public function insertData($input) {
        $data = $this->db->insert('album_asset', $input);
        if (!empty($data)) {
            $response['status'] = TRUE;
            $id = $this->db->insert_id();
            $q = $this->db->get_where('album_asset', array('asset_id' => $id));
            $response['data'] = $q->row();
        } else {
    ...
    public function updateData($id, $input) {
        $data = $this->db->update('album_asset', $input, array('asset_id' => $id));
    ...
    public function deleteData($id) {
        $data = $this->db->delete('album_asset', array('asset_id' => $id));
