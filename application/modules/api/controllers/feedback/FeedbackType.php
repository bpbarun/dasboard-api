 ...
 * @version         1.0.0
 */
class FeedbackType extends REST_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->database();
        $this->load->model('feedback/mFeedbackType');
    ...
    public function index_get($id = 0) {
        ...
        if (empty($responseData['error'])) {
            ...
            if (is_numeric($id)) {
                $data['id'] = $id;
                $response = $this->mFeedbackType->getData($data);
            } else {
                $data['id'] = explode(":", $id);
                $response = $this->mFeedbackType->getData($data);
            }
    ...
    public function index_post() {
        ...
        if (empty($responseData['error'])) {
            ...
            $response = $this->mFeedbackType->insertData($input);
            $this->response($response, REST_Controller::HTTP_OK);
        } else {
            $this->response($responseData, REST_Controller::HTTP_UNAUTHORIZED);
        }
    ...
    public function index_put($id) {
        ...
        if (empty($responseData['error'])) {
            ...
            $response = $this->mFeedbackType->updateData($id, $input);
            $this->response($response, REST_Controller::HTTP_OK);
        } else {
            $this->response($responseData, REST_Controller::HTTP_UNAUTHORIZED);
        }
    ...
    public function index_delete($id) {
        ...
        if (empty($responseData['error'])) {
            $input = $this->put();
            $input['modify_by'] = $responseData['data']['id'];
            $response = $this->mFeedbackType->deleteData($id);
            $this->response($response, REST_Controller::HTTP_OK);
        } else {
            $this->response($responseData, REST_Controller::HTTP_UNAUTHORIZED);
        }
