 ...
 * @subpackage      Controllers/Api/Event
 * @category        Api
 * @author          Barun Pandey
 * @date            11th April, 2020, 07:04:00 PM
 * @version         1.0.0
 */
   ...
   /**
    ...
    public function index_get($id = 0) {
        ...
        if (empty($responseData['error'])) {
            $response = $this->mOffer->getData($id);
    ...
    public function index_post() {
        ...
        if (empty($responseData['error'])) {
            $input = $this->post();
            $response = $this->mOffer->insertData($input);
            $this->response($response, REST_Controller::HTTP_OK);
        } else {
            $this->response($responseData, REST_Controller::HTTP_UNAUTHORIZED);
        }
    ...
    public function index_put($id) {
        ...
        if (empty($responseData['error'])) {
            $input = $this->put();
            $response = $this->mOffer->updateData($id, $input);
            $this->response($response, REST_Controller::HTTP_OK);
        } else {
            $this->response($responseData, REST_Controller::HTTP_UNAUTHORIZED);
        }
    ...
    public function index_delete($id) {
        ...
        if (empty($responseData['error'])) {
            $input = $this->put();
            $response = $this->mOffer->deleteData($id);
            $this->response($response, REST_Controller::HTTP_OK);
        } else {
            $this->response($responseData, REST_Controller::HTTP_UNAUTHORIZED);
        }
