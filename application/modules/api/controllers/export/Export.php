 ...
 * @subpackage      Controllers/Api/Export
 * @category        Api
 * @author          Barun Pandey
 * @date            24 August, 2019, 01:16:00 PM
 * @version         1.0.0
 */
class Export extends REST_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->database();
        $this->load->model('export/mExport');
    ...
    public function index_get($id = 0) {
        ...
        if (empty($responseData['error'])) {
            if (is_numeric($id)) {
                $response = $this->mExport->getData($id);
            } else {
                $data = explode(":", $id);
                $response = $this->mReport->getData($data);
            }
    ...
    public function index_post() {
        ...
        if (empty($responseData['error'])) {
            $input = $this->post();
            $response = $this->mExport->insertData($input);
            $this->response($response, REST_Controller::HTTP_OK);
        } else {
            $this->response($responseData, REST_Controller::HTTP_UNAUTHORIZED);
        }
    ...
    public function index_put($id) {
        ...
        if (empty($responseData['error'])) {
            $input = $this->put();
            $response = $this->mExport->updateData($id, $input);
            $this->response($response, REST_Controller::HTTP_OK);
        } else {
            $this->response($responseData, REST_Controller::HTTP_UNAUTHORIZED);
        }
    ...
    public function index_delete($id) {
        ...
        if (empty($responseData['error'])) {
            $input = $this->put();
            $response = $this->mExport->deleteData($id);
            $this->response($response, REST_Controller::HTTP_OK);
        } else {
            $this->response($responseData, REST_Controller::HTTP_UNAUTHORIZED);
        }
