...
class Report extends REST_Controller {
    ...
    public function index_get($id = 0) {
        ...
        if (empty($responseData['error'])) {
            if (is_numeric($id)) {
                $response = $this->mReport->getData($id);
            } else {
                $data = explode(":", $id);
    ...
    public function index_post() {
        ...
        if (empty($responseData['error'])) {
            $input = $this->post();
            $response = $this->mReport->insertData($input);
            $this->response($response, REST_Controller::HTTP_OK);
        } else {
            $this->response($responseData, REST_Controller::HTTP_UNAUTHORIZED);
        }
    ...
    public function index_put($id) {
        ...
        if (empty($responseData['error'])) {
            $input = $this->put();
            $response = $this->mReport->updateData($id, $input);
            $this->response($response, REST_Controller::HTTP_OK);
        } else {
            $this->response($responseData, REST_Controller::HTTP_UNAUTHORIZED);
        }
    ...
    public function index_delete($id) {
        ...
        if (empty($responseData['error'])) {
            $input = $this->put();
            $response = $this->mReport->deleteData($id);
            $this->response($response, REST_Controller::HTTP_OK);
        } else {
            $this->response($responseData, REST_Controller::HTTP_UNAUTHORIZED);
        }
